...
export interface BuddyCategoryHint {
  id: string;
  label: string;
  examples: string[];
}
...
export function parseBuddyCommand(
  ...
  if (options?.allowWake !== false && containsWakePhrase(text)) {
    ...
    const nested = parseBuddyCommand(withoutWake, categories, { allowWake: false });
  ...
  if (startMatch) {
    const activity = resolveActivity(startMatch[2], categories);
  ...
  const bareActivity = resolveActivity(text, categories);
